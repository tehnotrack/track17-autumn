...
public class Decoder
{
    ...
    Map<Character, Integer> createHist(@NotNull String text)
    {
        ...
        for(int i=0;i<text.length();++i)
        {
            ...
            if(CypherUtil.SYMBOLS.contains(Character.toString(c))) chars.add(c);
        }

        //Construct frequency map; It is needed for comparator constructing
        Map<Character, Integer> freq=new HashMap<>();
        for(int i=0;i<CypherUtil.SYMBOLS.length();++i) freq.put(CypherUtil.SYMBOLS.charAt(i), 0);
        for(char c :chars) freq.put(c, freq.get(c)+1);
