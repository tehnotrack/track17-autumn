...
public class Pool {
    ...
    private void broadcast(ServerConnection currentServerConnection, byte[] message, int size) throws IOException {
        for (ServerConnection serverConnection : serverConnections) {
            if (!serverConnection.equals(currentServerConnection)) {
                sendMessage(serverConnection, message, size);
            }
        }
    }
    
    private void sendMessage(ServerConnection serverConnection, byte[] message, int size) throws IOException {
        Socket       socket       = serverConnection.getSocket();
    ...
    public void addClient(Socket socket) throws IOException {
        ServerConnection serverConnection = new ServerConnection(
                id.getAndIncrement(), socket.getLocalAddress().toString(), socket.getPort(), socket);
    ...
    public boolean dropClient(int id) throws IOException, NoThreadSpecified {
        ...
        if (clientConnection != null) {
            Message message     = new Message("exit", "exit", System.currentTimeMillis());
            String  dropMessage = new Gson().toJson(message);
            
            sendMessage(clientConnection, dropMessage.getBytes(), dropMessage.length());
